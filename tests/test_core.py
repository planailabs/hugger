...
def test_verify_password_corrupt_hash_returns_false():
    """A malformed stored hash must fail the login gracefully, not raise (500)."""
    store.set_setting("password_hash", "not-a-valid-argon2-hash")
    try:
        assert auth.verify_password("anything") is False
    finally:
        auth.set_password("dbpw1234", autogenerated=False)  # restore a valid hash
