...
def test_change_password_flow():
    ...
    r = cli.post("/settings/password", data={"current": "bad", "new": "newpass1", "csrf": tok})
    assert "incorrect" in r.headers["location"]
    assert auth.verify_password(PASSWORD)
    # correct current updates it
    r = cli.post("/settings/password", data={"current": PASSWORD, "new": "newpass1", "csrf": tok})
...
def test_hf_token_set_and_clear():
    ...
    _login(cli)
    tok = _csrf(cli)
    r = cli.post("/settings/hf-token", data={"token": "hf_test123", "csrf": tok})
    assert "saved" in r.headers["location"]
    assert hub.current_hf_token() == "hf_test123"
    assert hub.hf_token_source() == "ui"
    cli.post("/settings/hf-token/clear", data={"csrf": tok})
...
def test_rotate_token_changes_token():
    ...
    cli.post("/settings/rotate-token", data={"csrf": _csrf(cli)})
    assert auth.cfg.api_token != before


def test_settings_forms_require_csrf():
    """The plain-form settings POSTs must reject a request with no CSRF token."""
    cli = _setup_client()
    _login(cli)
    before = auth.cfg.api_token
    r = cli.post("/settings/rotate-token")  # no csrf
    assert r.status_code == 303 and "Invalid+CSRF" in r.headers["location"]
    assert auth.cfg.api_token == before  # token unchanged
    r = cli.post("/settings/hf-token", data={"token": "hf_evil"})  # no csrf
    assert "Invalid+CSRF" in r.headers["location"]
