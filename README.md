...
```bash
python tests/test_core.py && python tests/test_api.py   # unit + HTTP integration
nix build .#checks.x86_64-linux.vm -L                    # full NixOS VM test
```

The VM test boots the NixOS module, changes the autogenerated password, and
downloads a model from a **fake HuggingFace Hub served over self-signed HTTPS** —
trusted by the VM via `security.pki.certificateFiles` and reached through an
`/etc/hosts` override of `huggingface.co` (see `nixos/test.nix` and
